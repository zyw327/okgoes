...
module.exports = class Request {
    ...
    getAction(options) {
        ...
        if (!this.isUseModule) {
            return urlPath.length > 1 ? urlPath[1] : options.defaultIndex;
        }
        return urlPath.length > 2 ? urlPath[2] : options.defaultIndex;
    }
    /**
     * @return {String}
     * @api public
    ...
    getController(options) {
        let urlPath = this.urlParse.getUrlPath();
        if (!this.isUseModule) {
            return urlPath.length ? this.toFirstUpper(urlPath[0]) : this.toFirstUpper(options.defaultController);
        }
        return urlPath.length > 1 ? this.toFirstUpper(urlPath[1]) : this.toFirstUpper(options.defaultController);
    }
    /**
     * @return {String}
     * @api public
